...
def run_multi_sample_vprPrep(vcf, fasta, cadd, out_path):
    """
    Multi-sample VPR-prep pipeline (one-step mode).
    """
    input_length = 10000
    max_tokens = 2000
    ...
    scores = []
    scores_with_cadd = []
    with open_maybe_gzip(cadd) as file, open(out_path, "w") as fout:
        ...
        for line in file:
            ...
            fout.write(f"{chrom}\t{target_pos}\t{ref_allele}\t{alt_allele}\t{risk_score}\t{cadd_score}\n")



def parse_args():
    parser = argparse.ArgumentParser(
        description="Run Multi-sample VPRPrep pipeline from a multi-sample VCF."
    )
    parser.add_argument(
        "--vcf",
        required=True,
        help="Multi-sample biallelic VCF (e.g. sample1.biallelic.vcf.gz)",
    )
    parser.add_argument(
        "--fasta",
        required=True,
    ...
    parser.add_argument(
        "--out",
        required=True,
        default="multisample_vpr.txt",
        help="Output file for this sample's VPRscore "
             "(e.g. ./multisample_vpr.txt)",
    )
